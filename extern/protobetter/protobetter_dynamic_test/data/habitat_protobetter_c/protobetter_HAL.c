...
uint32_t PackHAL_MBSU1(void *buffer, void *protobetter_type)
{
	...
	HAL_MBSU1 *mystruct = (HAL_MBSU1*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackMBSU_telemetry(&bytes[16], &mystruct->hal_mbsu1);
...
uint32_t UnpackHAL_MBSU1(void *protobetter_type, void *buffer)
{
	...
	UnpackMBSU_telemetry(&mystruct->hal_mbsu1, &bytes[16]);

	return sizeof(HAL_MBSU1);
}
...
uint32_t PackHAL_MBSU2(void *buffer, void *protobetter_type)
{
	...
	HAL_MBSU2 *mystruct = (HAL_MBSU2*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackMBSU_telemetry(&bytes[16], &mystruct->hal_mbsu2);
...
uint32_t UnpackHAL_MBSU2(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	UnpackMBSU_telemetry(&mystruct->hal_mbsu2, &bytes[16]);

	return sizeof(HAL_MBSU2);
}
...
uint32_t PackHAL_PDU1(void *buffer, void *protobetter_type)
{
	...
	HAL_PDU1 *mystruct = (HAL_PDU1*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackPDU_telemetry(&bytes[16], &mystruct->hal_pdu1);
...
uint32_t UnpackHAL_PDU1(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	UnpackPDU_telemetry(&mystruct->hal_pdu1, &bytes[16]);

	return sizeof(HAL_PDU1);
}
...
uint32_t PackHAL_PDU2(void *buffer, void *protobetter_type)
{
	...
	HAL_PDU2 *mystruct = (HAL_PDU2*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackPDU_telemetry(&bytes[16], &mystruct->hal_pdu2);
...
uint32_t UnpackHAL_PDU2(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	UnpackPDU_telemetry(&mystruct->hal_pdu2, &bytes[16]);

	return sizeof(HAL_PDU2);
}
...
uint32_t PackHAL_PDU3(void *buffer, void *protobetter_type)
{
	...
	HAL_PDU3 *mystruct = (HAL_PDU3*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackPDU_telemetry(&bytes[16], &mystruct->hal_pdu3);
...
uint32_t UnpackHAL_PDU3(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	UnpackPDU_telemetry(&mystruct->hal_pdu3, &bytes[16]);

	return sizeof(HAL_PDU3);
}
...
uint32_t PackHAL_PDU4(void *buffer, void *protobetter_type)
{
	...
	HAL_PDU4 *mystruct = (HAL_PDU4*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	PackPDU_telemetry(&bytes[16], &mystruct->hal_pdu4);
...
uint32_t UnpackHAL_PDU4(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	UnpackPDU_telemetry(&mystruct->hal_pdu4, &bytes[16]);

	return sizeof(HAL_PDU4);
}
...
uint32_t PackHAL_Power_Source1(void *buffer, void *protobetter_type)
{
	...
	HAL_Power_Source1 *mystruct = (HAL_Power_Source1*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	Packsim_power_source(&bytes[16], &mystruct->hal_power_source1);
...
uint32_t UnpackHAL_Power_Source1(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	Unpacksim_power_source(&mystruct->hal_power_source1, &bytes[16]);

	return sizeof(HAL_Power_Source1);
}
...
uint32_t PackHAL_Power_Source2(void *buffer, void *protobetter_type)
{
	...
	HAL_Power_Source2 *mystruct = (HAL_Power_Source2*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;

	memcpy(&bytes[0], &mystruct->cfsHeader, 16);
	Packsim_power_source(&bytes[16], &mystruct->hal_power_source2);
...
uint32_t UnpackHAL_Power_Source2(void *protobetter_type, void *buffer)
{
	...
	memcpy(&mystruct->cfsHeader, &bytes[0], 16);
	Unpacksim_power_source(&mystruct->hal_power_source2, &bytes[16]);

	return sizeof(HAL_Power_Source2);
}
...
uint32_t PackSET_HAL_MBSU1_INPUT_SOURCE_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU1_INPUT_SOURCE_SELECT_t *mystruct = (SET_HAL_MBSU1_INPUT_SOURCE_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU1_INPUT_SOURCE_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU1_INPUT_SOURCE_SELECT_t);
}
...
uint32_t PackSET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t *mystruct = (SET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_MBSU1_Input_Curr_Limit_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU1_Input_Curr_Limit_t *mystruct = (SET_HAL_MBSU1_Input_Curr_Limit_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU1_Input_Curr_Limit_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU1_Input_Curr_Limit_t);
}
...
uint32_t PackSET_HAL_MBSU1_Output_Curr_Limit_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU1_Output_Curr_Limit_t *mystruct = (SET_HAL_MBSU1_Output_Curr_Limit_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU1_Output_Curr_Limit_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU1_Output_Curr_Limit_t);
}
...
uint32_t PackCLEAR_HAL_MBSU1_Input_Curr_Trip_t(void *buffer, void *protobetter_type)
{
	...
	CLEAR_HAL_MBSU1_Input_Curr_Trip_t *mystruct = (CLEAR_HAL_MBSU1_Input_Curr_Trip_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackCLEAR_HAL_MBSU1_Input_Curr_Trip_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(CLEAR_HAL_MBSU1_Input_Curr_Trip_t);
}
...
uint32_t PackCLEAR_HAL_MBSU1_Output_Curr_Trip_t(void *buffer, void *protobetter_type)
{
	...
	CLEAR_HAL_MBSU1_Output_Curr_Trip_t *mystruct = (CLEAR_HAL_MBSU1_Output_Curr_Trip_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackCLEAR_HAL_MBSU1_Output_Curr_Trip_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(CLEAR_HAL_MBSU1_Output_Curr_Trip_t);
}
...
uint32_t PackSET_HAL_MBSU1_Relay_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU1_Relay_CHANNEL_OPEN_t *mystruct = (SET_HAL_MBSU1_Relay_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU1_Relay_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU1_Relay_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_MBSU1_FAIL_INPUT_SOURCE_t(void *buffer, void *protobetter_type)
{
	...
	HAL_MBSU1_FAIL_INPUT_SOURCE_t *mystruct = (HAL_MBSU1_FAIL_INPUT_SOURCE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackHAL_MBSU1_FAIL_INPUT_SOURCE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(HAL_MBSU1_FAIL_INPUT_SOURCE_t);
}
...
uint32_t PackHAL_MBSU1_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return PackSET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_MBSU1_INPUT_SOURCE_SELECT_t(buffer, protobetter_type);

		case 0x7:

			return PackHAL_MBSU1_FAIL_INPUT_SOURCE_t(buffer, protobetter_type);

		case 0x2:

			return PackSET_HAL_MBSU1_Input_Curr_Limit_t(buffer, protobetter_type);

		case 0x3:

			return PackSET_HAL_MBSU1_Output_Curr_Limit_t(buffer, protobetter_type);

		case 0x6:

			return PackSET_HAL_MBSU1_Relay_CHANNEL_OPEN_t(buffer, protobetter_type);

		case 0x5:

			return PackCLEAR_HAL_MBSU1_Output_Curr_Trip_t(buffer, protobetter_type);

		case 0x4:

			return PackCLEAR_HAL_MBSU1_Input_Curr_Trip_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_MBSU1_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return UnpackSET_HAL_MBSU1_OUTPUT_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_MBSU1_INPUT_SOURCE_SELECT_t(protobetter_type, buffer);

		case 0x7:

			return UnpackHAL_MBSU1_FAIL_INPUT_SOURCE_t(protobetter_type, buffer);

		case 0x2:

			return UnpackSET_HAL_MBSU1_Input_Curr_Limit_t(protobetter_type, buffer);

		case 0x3:

			return UnpackSET_HAL_MBSU1_Output_Curr_Limit_t(protobetter_type, buffer);

		case 0x6:

			return UnpackSET_HAL_MBSU1_Relay_CHANNEL_OPEN_t(protobetter_type, buffer);

		case 0x5:

			return UnpackCLEAR_HAL_MBSU1_Output_Curr_Trip_t(protobetter_type, buffer);

		case 0x4:

			return UnpackCLEAR_HAL_MBSU1_Input_Curr_Trip_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_MBSU2_INPUT_SOURCE_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU2_INPUT_SOURCE_SELECT_t *mystruct = (SET_HAL_MBSU2_INPUT_SOURCE_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU2_INPUT_SOURCE_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU2_INPUT_SOURCE_SELECT_t);
}
...
uint32_t PackSET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t *mystruct = (SET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_MBSU2_Input_Curr_Limit_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU2_Input_Curr_Limit_t *mystruct = (SET_HAL_MBSU2_Input_Curr_Limit_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU2_Input_Curr_Limit_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU2_Input_Curr_Limit_t);
}
...
uint32_t PackSET_HAL_MBSU2_Output_Curr_Limit_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU2_Output_Curr_Limit_t *mystruct = (SET_HAL_MBSU2_Output_Curr_Limit_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU2_Output_Curr_Limit_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU2_Output_Curr_Limit_t);
}
...
uint32_t PackCLEAR_HAL_MBSU2_Input_Curr_Trip_t(void *buffer, void *protobetter_type)
{
	...
	CLEAR_HAL_MBSU2_Input_Curr_Trip_t *mystruct = (CLEAR_HAL_MBSU2_Input_Curr_Trip_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackCLEAR_HAL_MBSU2_Input_Curr_Trip_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(CLEAR_HAL_MBSU2_Input_Curr_Trip_t);
}
...
uint32_t PackCLEAR_HAL_MBSU2_Output_Curr_Trip_t(void *buffer, void *protobetter_type)
{
	...
	CLEAR_HAL_MBSU2_Output_Curr_Trip_t *mystruct = (CLEAR_HAL_MBSU2_Output_Curr_Trip_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackCLEAR_HAL_MBSU2_Output_Curr_Trip_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(CLEAR_HAL_MBSU2_Output_Curr_Trip_t);
}
...
uint32_t PackSET_HAL_MBSU2_Relay_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_MBSU2_Relay_CHANNEL_OPEN_t *mystruct = (SET_HAL_MBSU2_Relay_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_MBSU2_Relay_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_MBSU2_Relay_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_MBSU2_FAIL_INPUT_SOURCE_t(void *buffer, void *protobetter_type)
{
	...
	HAL_MBSU2_FAIL_INPUT_SOURCE_t *mystruct = (HAL_MBSU2_FAIL_INPUT_SOURCE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackHAL_MBSU2_FAIL_INPUT_SOURCE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(HAL_MBSU2_FAIL_INPUT_SOURCE_t);
}
...
uint32_t PackHAL_MBSU2_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x2:
			...
			return PackSET_HAL_MBSU2_Input_Curr_Limit_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_MBSU2_INPUT_SOURCE_SELECT_t(buffer, protobetter_type);

		case 0x7:

			return PackHAL_MBSU2_FAIL_INPUT_SOURCE_t(buffer, protobetter_type);

		case 0x1:

			return PackSET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x3:

			return PackSET_HAL_MBSU2_Output_Curr_Limit_t(buffer, protobetter_type);

		case 0x5:

			return PackCLEAR_HAL_MBSU2_Output_Curr_Trip_t(buffer, protobetter_type);

		case 0x6:

			return PackSET_HAL_MBSU2_Relay_CHANNEL_OPEN_t(buffer, protobetter_type);

		case 0x4:

			return PackCLEAR_HAL_MBSU2_Input_Curr_Trip_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_MBSU2_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x2:
			...
			return UnpackSET_HAL_MBSU2_Input_Curr_Limit_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_MBSU2_INPUT_SOURCE_SELECT_t(protobetter_type, buffer);

		case 0x7:

			return UnpackHAL_MBSU2_FAIL_INPUT_SOURCE_t(protobetter_type, buffer);

		case 0x1:

			return UnpackSET_HAL_MBSU2_OUTPUT_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x3:

			return UnpackSET_HAL_MBSU2_Output_Curr_Limit_t(protobetter_type, buffer);

		case 0x5:

			return UnpackCLEAR_HAL_MBSU2_Output_Curr_Trip_t(protobetter_type, buffer);

		case 0x6:

			return UnpackSET_HAL_MBSU2_Relay_CHANNEL_OPEN_t(protobetter_type, buffer);

		case 0x4:

			return UnpackCLEAR_HAL_MBSU2_Input_Curr_Trip_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PDU1_INPUT_BUS_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU1_INPUT_BUS_SELECT_t *mystruct = (SET_HAL_PDU1_INPUT_BUS_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU1_INPUT_BUS_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU1_INPUT_BUS_SELECT_t);
}
...
uint32_t PackSET_HAL_PDU1_RPC_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU1_RPC_CHANNEL_STATE_t *mystruct = (SET_HAL_PDU1_RPC_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU1_RPC_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU1_RPC_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PDU1_RPC_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU1_RPC_CHANNEL_OPEN_t *mystruct = (SET_HAL_PDU1_RPC_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU1_RPC_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU1_RPC_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_PDU1_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return PackSET_HAL_PDU1_RPC_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_PDU1_INPUT_BUS_SELECT_t(buffer, protobetter_type);

		case 0x2:

			return PackSET_HAL_PDU1_RPC_CHANNEL_OPEN_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_PDU1_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return UnpackSET_HAL_PDU1_RPC_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_PDU1_INPUT_BUS_SELECT_t(protobetter_type, buffer);

		case 0x2:

			return UnpackSET_HAL_PDU1_RPC_CHANNEL_OPEN_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PDU2_INPUT_BUS_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU2_INPUT_BUS_SELECT_t *mystruct = (SET_HAL_PDU2_INPUT_BUS_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU2_INPUT_BUS_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU2_INPUT_BUS_SELECT_t);
}
...
uint32_t PackSET_HAL_PDU2_RPC_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU2_RPC_CHANNEL_STATE_t *mystruct = (SET_HAL_PDU2_RPC_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU2_RPC_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU2_RPC_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PDU2_RPC_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU2_RPC_CHANNEL_OPEN_t *mystruct = (SET_HAL_PDU2_RPC_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU2_RPC_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU2_RPC_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_PDU2_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return PackSET_HAL_PDU2_RPC_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x2:

			return PackSET_HAL_PDU2_RPC_CHANNEL_OPEN_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_PDU2_INPUT_BUS_SELECT_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_PDU2_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return UnpackSET_HAL_PDU2_RPC_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x2:

			return UnpackSET_HAL_PDU2_RPC_CHANNEL_OPEN_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_PDU2_INPUT_BUS_SELECT_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PDU3_INPUT_BUS_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU3_INPUT_BUS_SELECT_t *mystruct = (SET_HAL_PDU3_INPUT_BUS_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU3_INPUT_BUS_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU3_INPUT_BUS_SELECT_t);
}
...
uint32_t PackSET_HAL_PDU3_RPC_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU3_RPC_CHANNEL_STATE_t *mystruct = (SET_HAL_PDU3_RPC_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU3_RPC_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU3_RPC_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PDU3_RPC_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU3_RPC_CHANNEL_OPEN_t *mystruct = (SET_HAL_PDU3_RPC_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU3_RPC_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU3_RPC_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_PDU3_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return PackSET_HAL_PDU3_RPC_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x2:

			return PackSET_HAL_PDU3_RPC_CHANNEL_OPEN_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_PDU3_INPUT_BUS_SELECT_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_PDU3_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return UnpackSET_HAL_PDU3_RPC_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x2:

			return UnpackSET_HAL_PDU3_RPC_CHANNEL_OPEN_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_PDU3_INPUT_BUS_SELECT_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PDU4_INPUT_BUS_SELECT_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU4_INPUT_BUS_SELECT_t *mystruct = (SET_HAL_PDU4_INPUT_BUS_SELECT_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU4_INPUT_BUS_SELECT_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU4_INPUT_BUS_SELECT_t);
}
...
uint32_t PackSET_HAL_PDU4_RPC_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU4_RPC_CHANNEL_STATE_t *mystruct = (SET_HAL_PDU4_RPC_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU4_RPC_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU4_RPC_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PDU4_RPC_CHANNEL_OPEN_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PDU4_RPC_CHANNEL_OPEN_t *mystruct = (SET_HAL_PDU4_RPC_CHANNEL_OPEN_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PDU4_RPC_CHANNEL_OPEN_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PDU4_RPC_CHANNEL_OPEN_t);
}
...
uint32_t PackHAL_PDU4_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x2:
			...
			return PackSET_HAL_PDU4_RPC_CHANNEL_OPEN_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_PDU4_INPUT_BUS_SELECT_t(buffer, protobetter_type);

		case 0x1:

			return PackSET_HAL_PDU4_RPC_CHANNEL_STATE_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_PDU4_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x2:
			...
			return UnpackSET_HAL_PDU4_RPC_CHANNEL_OPEN_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_PDU4_INPUT_BUS_SELECT_t(protobetter_type, buffer);

		case 0x1:

			return UnpackSET_HAL_PDU4_RPC_CHANNEL_STATE_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PS1_OUTPUT_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PS1_OUTPUT_CHANNEL_STATE_t *mystruct = (SET_HAL_PS1_OUTPUT_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PS1_OUTPUT_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PS1_OUTPUT_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PS1_SAR_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PS1_SAR_STATE_t *mystruct = (SET_HAL_PS1_SAR_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PS1_SAR_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PS1_SAR_STATE_t);
}
...
uint32_t PackHAL_Power_Source1_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x0:
			...
			return PackSET_HAL_PS1_OUTPUT_CHANNEL_STATE_t(buffer, protobetter_type);

		case 0x1:

			return PackSET_HAL_PS1_SAR_STATE_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_Power_Source1_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x0:
			...
			return UnpackSET_HAL_PS1_OUTPUT_CHANNEL_STATE_t(protobetter_type, buffer);

		case 0x1:

			return UnpackSET_HAL_PS1_SAR_STATE_t(protobetter_type, buffer);
...
uint32_t PackSET_HAL_PS2_OUTPUT_CHANNEL_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PS2_OUTPUT_CHANNEL_STATE_t *mystruct = (SET_HAL_PS2_OUTPUT_CHANNEL_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PS2_OUTPUT_CHANNEL_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PS2_OUTPUT_CHANNEL_STATE_t);
}
...
uint32_t PackSET_HAL_PS2_SAR_STATE_t(void *buffer, void *protobetter_type)
{
	...
	SET_HAL_PS2_SAR_STATE_t *mystruct = (SET_HAL_PS2_SAR_STATE_t*)protobetter_type;
	unsigned char *bytes = (unsigned char*)buffer;
...
uint32_t UnpackSET_HAL_PS2_SAR_STATE_t(void *protobetter_type, void *buffer)
{
	...
	return sizeof(SET_HAL_PS2_SAR_STATE_t);
}
...
uint32_t PackHAL_Power_Source2_CMD(void *buffer, void *protobetter_type)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return PackSET_HAL_PS2_SAR_STATE_t(buffer, protobetter_type);

		case 0x0:

			return PackSET_HAL_PS2_OUTPUT_CHANNEL_STATE_t(buffer, protobetter_type);
...
uint32_t UnpackHAL_Power_Source2_CMD(void *protobetter_type, void *buffer)
{
	...
	switch (cmdCode)
	{
		case 0x1:
			...
			return UnpackSET_HAL_PS2_SAR_STATE_t(protobetter_type, buffer);

		case 0x0:

			return UnpackSET_HAL_PS2_OUTPUT_CHANNEL_STATE_t(protobetter_type, buffer);
